...
class ArraysTest < Minitest::Test
  ...
  def test_2
    ...
    # actual = nums.delete(2) ## Solution 1
    actual = nums.at(1) ## Solution 2
  ...
  def test_4
    ...
    world_cup.insert(-1, "Argentina")
    assert_equal "Argentina", world_cup.last

    # Use a different method to add the element "Japan" to the end of the array
    world_cup << "Japan"
    assert_equal "Japan", world_cup.last
    world_cup.unshift("USA")
    assert_equal "USA", world_cup.first
    world_cup.insert(0, semifinalists: ["Mexico", "Korea", "Germany"])
    
    expected = {semifinalists: ["Mexico", "Korea", "Germany"]}
    assert_equal expected, world_cup.first
    assert_kind_of Hash, world_cup.first
  end
  ...
  def test_a
    ...
    assert_equal "Iceland", last_element
    assert_equal ["Germany", "Mexico"], world_cup
  end
  ...
  def test_5
    ...
    dancing_queen_in_array2 = karaoke.one?("Dancing Queen")
    assert_equal true, dancing_queen_in_array
  end
  ...
  def test_b
    ...
    band.insert(0, "Piano")
    assert_equal ["Piano","Vocals", "Guitar", "Drums", "Bass"], band
  end
  ...
  def test_9
    ...
    new_scores = scores
    assert_equal [48, 23, 56, 34, 29], new_scores.rotate(-1)
  end
